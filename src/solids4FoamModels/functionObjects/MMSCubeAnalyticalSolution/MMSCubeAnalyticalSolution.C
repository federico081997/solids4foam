...
Foam::scalar Foam::MMSCubeAnalyticalSolution::MMSCubePressure
(
    const vector& point
)
{
    // Bulk modulus
    const scalar kappa = E_/(3.0*(1.0 - 2*nu_));
    ...
    return scalar
    (
        -kappa*(4*ax_*pi*cos(4*pi*point.x())*sin(2*pi*point.y())*sin(pi*point.z()) + 2*ay_*pi*cos(2*pi*point.y())*sin(4*pi*point.x())*sin(pi*point.z()) + az_*pi*cos(pi*point.z())*sin(4*pi*point.x())*sin(2*pi*point.y())) 
    );
}
...
bool Foam::MMSCubeAnalyticalSolution::writeData()
{
    ...
    if (pointDisplacement_ || pointStress_ || pointEpsilon_)
    {
        ...
        pointScalarField analyticalPointP
        (
            IOobject
            (
                "analyticalPointP",
        ...
        scalarField& aPI = analyticalPointP;
        ...
        forAll(aPI, pointI)
        {
            aPI[pointI] = MMSCubePressure(points[pointI]);
        }
        ...
        Info<< "Writing analyticalPointP"
            << nl << endl;
        analyticalPointP.write();
        ...
        )
        {
            ...
            //{
            //pointSigmaEq[pointI] =
            //sqrt((sqr(pointSigma[pointI].component(symmTensor::XX) - pointSigma[pointI].yy()) + sqr(pointSigma[pointI].yy() - pointSigma[pointI].zz()) +
...
//                << nl << endl;
        ...
        if
        (
            mesh.foundObject<pointScalarField>("pointP")
        )
        {
            const pointScalarField& pointP =
                mesh.lookupObject<pointScalarField>("pointP");

            const pointScalarField diff
            (
                "pointPDifference", analyticalPointP - pointP
            );
            Info<< "Writing pointPDifference field" << endl;
            diff.write();

            const scalarField& diffI = diff;
            Info<< "    Pressure norms: mean L1, mean L2, LInf: " << nl
                << "    " << gAverage(mag(diffI))
                << " " << Foam::sqrt(gAverage(magSqr(diffI)))
                << " " << gMax(mag(diffI))
                << nl << endl;
